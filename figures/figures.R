...
cairo(file='~/Documents/temp/ThUisochron.pdf',height=6,width=6)
pars(mfrow=c(2,2),mar=c(2.5,2.5,3,0.5))
isochron(examples$ThU,type=1)
legend('topleft',legend='a)',bty='n',adj=c(1.5,0),cex=1.2)
isochron(examples$ThU,type=2)
legend('topleft',legend='b)',bty='n',adj=c(1.5,0),cex=1.2)
isochron(examples$ThU,type=3)
legend('topleft',legend='c)',bty='n',adj=c(1.5,0),cex=1.2)
isochron(examples$ThU,type=4)
legend('topleft',legend='d)',bty='n',adj=c(1.5,0),cex=1.2)
dev.off()

cairo(file='~/Documents/temp/evolution12.pdf',height=2.5,width=7.5)
pars(mfrow=c(1,3),mar=c(2.5,2.85,0.1,0.5))
evolution(examples$ThU,isochron=TRUE)
legend('topright',legend='a)',bty='n',adj=c(0,0),cex=1.3)
evolution(examples$ThU,isochron=FALSE,Th0i=1)
legend('topright',legend='b)',bty='n',adj=c(0,0),cex=1.3)
evolution(examples$ThU,transform=TRUE,Th0i=1)
legend('topright',legend='c)',bty='n',adj=c(0,0),cex=1.3)
dev.off()

cairo(file='~/Documents/temp/evolution34.pdf',height=3,width=6)
pars(mfrow=c(1,2),mar=c(2.5,2.85,0.1,0.5))
fn <- system.file('ThU3.csv',package='IsoplotR')
ThU <- read.data(fn,method='Th-U',format=3)
evolution(ThU)
legend('topleft',legend='a)',bty='n',adj=c(1.5,0),cex=1.2)
ThUb <- read.data(fn,method='Th-U',format=3,U8Th2=2)
evolution(ThUb,Th0i=2)
legend('topleft',legend='b)',bty='n',adj=c(1.5,0),cex=1.2)
dev.off()

cairo(file='~/Documents/temp/297.pdf',height=6,width=6)
pars(mfrow=c(2,2))
Gehrels <- read.data('297Gehrels.csv',method='U-Pb',format=1,ierr=3)
Rasmussen <- read.data('297Rasmussen.csv',method='U-Pb',format=1,ierr=4)
radialplot(Gehrels,k='min',omit=1,
           cutoff.disc=discfilter(option=5,cutoff=c(-1,2)))
radialplot(Rasmussen,k='min')
dev.off()
...
pars(mfrow=c(1,2),mar=c(3,3,3.5,1))
evolution(examples$ThU,transform=TRUE,Th0i=0,isochron=TRUE)
evolution(examples$ThU,transform=TRUE,Th0i=1,isochron=TRUE)
dev.off()

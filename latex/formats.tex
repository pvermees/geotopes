...
elsewhere.
\begin{equation}
  MSWD = \sum\limits_{i=1}^{n}\frac{(t_i-\bar{t}_w)^2}{(n-1) s[t_i]^2}
  \label{eq:mswd}
\end{equation}
...
\begin{minipage}[t]{.58\textwidth}
  \captionof{figure}{Three different synthetic data
    sets shown as weighted mean plots. The analytical uncertainties
    are shown as 95\% error bars. Each of the data sets consists of 10
...
\item\textbf{Model-2: ignore the analytical uncertainties}
  ...
  point of view, the physical meaning of the overdispersion factor are
  ...
  might wonder why the degree of unreliability should be the same for
  all aliquots. An alternative way to deal with unreliable uncertainty
  estimates is to ignore them altogether and use the ordinary
  arithmetic mean as a fallback solution.
...
CHANGE TO CONVENTIONAL ISOCHRON:
...
\end{minipage}
\begin{minipage}[t]{.5\textwidth}
  \vspace{-20pt} \captionof{figure}{Three different synthetic

...
\item formats 4--6: Total-U/Pb regression using
  \textsuperscript{204/6/7}Pb and \textsuperscript{235/8}U
...
\item formats 7 and 8: Total-U-Th/Pb regression using
  \textsuperscript{206/7/8}Pb \textsuperscript{232}Th and
  \textsuperscript{235/8}U \citep{vermeesch2020}. If
...
\item In igneous and ortho-metamorphic rocks, where one can safely
  ...
  \end{enumerate}
\item In sedimentary and para-metamorphic rocks, in which the grains
  cannot safely be assumed to be cogenetic, each aliquot must be
  treated separately.
  \begin{enumerate}
  \item If there is some independent evidence to support a shared
    common-Pb composition for all the aliquots, then the radiogenic
    composition can be obtained by two-point isochron regression
    between that common-Pb composition and each aliquot.\\
    ...
    \begin{minipage}[t]{\linewidth}
      \centering \captionsetup{width=.8\textwidth} \captionof{figure}{
        ...
        composition with
        \textsuperscript{206}Pb/\textsuperscript{206}Pb = 0.7308.
        Each aliquot forms its own mixing line, and its uncertainties
        are magnified by the extrapolation, in proportion to its
        distance to the concordia line. This is different from the
        joint isochron regression of
        Figures~\ref{fig:commonPbisochron13} and
        \ref{fig:commonPbisochron46}, in which all aliquots are
  ...
  \item In the absence of external evidence about the common Pb
    composition, one can infer the composition from the age using a
    mantle evolution model \citep[e.g.,][]{stacey1975}. If $t<3.7$~Ga:
    \begin{align}

...
Chapter~\ref{ch:generic-R}, are included in \texttt{IsoplotR} and
whose file path can be found with the \texttt{system.file()}
function. \texttt{read.data()} returns a list with three items:
...
normal distribution (Figure~\ref{fig:norm2dmarginal}):
\end{minipage}
...
\begin{console}
concordia(UPb3,levels=ThU,ellipse.fill=c(rgb(1,0,0,0.5),rgb(0,1,0,0.5)))
\end{console}

...
1. Consider the following three aliquots from the same sample:
   ...
   tabUPb <- signif(cbind(Pb64,Pb74,U8Pb4),3)
   ...
   U-Pb composion in concordia space).
...
1. Consider the Pb-Pb composition of three aliquots from a meteorite:
   ...
   Hint: think isochrons.
   ```{r include=solution, out.width='60%', fig.width=5, fig.height=4}
   Pb64 <- PbPbtab[1,]
   Pb74 <- PbPbtab[2,]
...
Solary System history contain measurable amounts of excess of ^129^Xe.
   ...
   \frac{\left[\frac{{}^{129}Xe}{{}^{127}I}\right]_A}
       {\left[\frac{{}^{129}Xe}{{}^{127}I}\right]_B} =
	 \exp[\lambda_{129}(t_A-t_B)]
   \]
   Hence
   \[
   t_A-t_B = \frac{1}{\lambda_{129}}
   \left(
...
1. Given the ^40^Ar/^39^Ar-data shown in the following table:
   ...
   is of atmospheric origin ($[$^40^Ar/^36^Ar$]$~atm~=298.5); whereas
   'smp1', 'smp2' and 'smp3' are three cogenetic aliquots of a
   ...
   ```{r include=solution, out.width='50%', fig.width=5, fig.height=4}
...
1. Analysis of a granite yields:
   ...
   very rough and ready as can be seen!
   \fi
...
apatite data:
   ...
   | average fission track length | 15 \textmu m |
   | U-concentration | 10 ppm |
   | density of apatite | 3.19 g/cm^3^ |
   | molar mass of U | 238.04 g/mol |
...
1. The ^234^U/^238^U activity ratio of sea water is rarely exceeds
...
replaced by mass specrometry. Consider the following composition of
three cogenetic aliquots:
   ...
   tabThU <- cbind(Uppm,Thppm,
   format(Th02,digits=4,scientific=FALSE))
   ...
   How old is the rock?
   ```{r include=solution}
   # 1. store the data into three variables:
   Uppm
   Thppm
   Th02
   # 2. convert Uppm and Thppm to activities:
   l238 <- 1.55125e-7 # kyr-1
   l232 <- 4.95e-8 # kyr-1
   MU <- 238.03
   MTh <- 232.04
   ...
   Navog <- 6.022e23
   AU238dpmg <- AU238*Navog/(1e4*365*24*60) # decay events per minute per gram
   ATh232dpmg <- ATh232*Navog/(1e4*365*24*60) # decay events per minute per gram
   message('A(U238)=',signif(AU238dpmg,3),' decay events per minute and gram')
   message('A(Th232)=',signif(ATh232dpmg,3),' decay events per minute and gram')
   # 3. compute the activity ratios:
   A82 <- U238mumolg/Th232mumolg
   Th230mumolg <- Th232mumolg*Th02
   l230 <- 0.0091705 # kyr-1
   ATh230 <- Th230mumolg*l230
   A02 <- ATh230/ATh232
   # 4. plot on an isochron diagram and find the slope:
   plot(x=A82,y=A02,type='b',xlab='U238/Th232',ylab='Th230/Th232')
   fit <- lm(A02 ~ A82)$coefficients[2]
   # alternatively:
   slope <- diff(A02)/diff(A82)
   # 5. compute the age:

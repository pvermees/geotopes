...
1. Install the `IsoplotRgui` package and start it using the
`IsoplotR()` function. Explore the Rb-Sr, Sm-Nd, Re-Os and Lu-Hf
functions.

## U-Pb

1. Plot the U-Pb composition of sample 91500 on a Wetherill concordia
diagram.
...
1. Explore the U-Pb functionality of the `IsoplotRgui` package.
Change the options to compare different input formats. Each row in the
example datasets represents an isotopic composition that was computed
from mass spectrometer data using software that is similar to the `R`
code that you wrote to process sample 91500. Plot data as Wetherill
and Tera-Wasserburg concordia diagrams.
...
1. Explore the ^40^Ar/^39^Ar-functionality of `IsoplotRgui`, including
the plateau and isochron functions.
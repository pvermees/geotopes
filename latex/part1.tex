...
geological interpretations based on isotopic methods. Isotope
...
which both van den Haute and \citet{allegre2008} heavily
borrowed. Additional lecture material, including the data files used
in the \texttt{Matlab} practicals of Section~\ref{sec:programming},
can be found at \texttt{http://github.com/pvermees/geotopes}.
...
tutorials of the most basic functionality in \texttt{R} and
\texttt{Matlab}, respectively.  These will cover most commands that
...
# Change the third value in the second row of mymat to 5:
mymat[2,3] <- 5

# Change the entire second column of mymat to 0:
mymat[,2] <- 0
...
\item \texttt{Matlab}, like \texttt{R}, allows the user to run code
  ...
  your code with lots of comments, as will be done for this tutorial.
...
Copy the \texttt{coinToss} function into a text file named
\texttt{coinToss.m} and save it in the current directory. You can then
load the contents of the file by simply typing its name without the
\texttt{.m} extension:
